...
import { fmtISO, addDays } from '../services/periodUtils';
...
router.get('/team-roster', requireAuth, requireTenantContext, async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Fetch roster shift segments for all employees in this fortnight
        const dateList = days.map(d => `'${d.date}'`).join(', ');
        const recordsRes = await query(
            `SELECT dr.id as record_id, dr.employee_id, dr.record_date,
             ...
             WHERE dr.org_id = $1 AND dr.record_date IN (${dateList})`,
            [orgId]
        );
